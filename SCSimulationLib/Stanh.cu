...
namespace scsim {
	...
	Stanh::Stanh(uint32_t input, uint32_t output, uint32_t k, StochasticCircuitFactory* factory) : SequentialComponent(1, 1, 0, typehash(Stanh), sizeof(Stanh), alignof(Stanh), factory), k(k) {
		inputs_host[0] = input;
