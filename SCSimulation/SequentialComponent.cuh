...
namespace scsim {
	...
	public:
		...
		SequentialComponent(uint32_t num_inputs, uint32_t num_outputs, uint32_t type, size_t size, size_t align, StochasticCircuitFactory* factory) :
			CircuitComponent(num_inputs, num_outputs, type | 0x80000000u, size, align, factory) { //sequential component types have upper bit set -> sequential components always come last once sorted
